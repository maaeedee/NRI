...
from utils_new import my_softmax, get_offdiag_indices, gumbel_softmax
...
class MLPEncoder(nn.Module):
    def __init__(self, n_in, n_hid, n_out, do_prob=0., factor=True):
        ...
        self.mlp1 = MLP(n_in, n_hid, n_hid, do_prob) #Equation(5) in the paper: h^1_j = femb(x_j)
    ...
    def edge2node(self, x, rel_rec, rel_send):
        ...
        incoming = torch.matmul(rel_rec.t(), x) #|V|* he
    ...
    def node2edge(self, x, rel_rec, rel_send):
        ...
        G = (V, E)
        x: embedding vectors of nodes: |V|*hn (hn: dimensions of each node embedding)
        rel_rec: Matrix denoting incomming edges of nodes: |E|*|V|
    ...
    def forward(self, inputs, rel_rec, rel_send):
        ...
        # num_sims = batch_size, num_atoms = number_of_nodes?
        x = inputs.view(inputs.size(0), inputs.size(1), -1)
        ...
        x = self.mlp2(x)
        x_skip = x #x_skip: edge embeddings
...
class CNNEncoder(nn.Module):
    def __init__(self, n_in, n_hid, n_out, do_prob=0., factor=True):
        """
          n_in: num_dims(features in each timestep)
...
class MLPDecoder(nn.Module):
    ...
    def __init__(self, n_in_node, edge_types, msg_hid, msg_out, n_hid,
        ...
        """
          n_in_nodes: number of dimensions
          edge_types: number of edge types to infer
          msg_hid: number of hidden units
          msg_out: number of hidden units
    ...
    def single_step_forward(self, single_timestep_inputs, rel_rec, rel_send,
        ...
        # to contain the probaility of each edge types for every edge of every
        # simulation and timesteps

        #rel_rec: shape: [num_edges, num_atoms]

        # Node2edge
        receivers = torch.matmul(rel_rec, single_timestep_inputs)
        #receivers shape [batch_size, num_timesteps, num_edges, num_dims]
        senders = torch.matmul(rel_send, single_timestep_inputs)
        #senders shape: [batch_size, num_timesteps, num_edges, num_dims]

        pre_msg = torch.cat([senders, receivers], dim=-1)
        #pre_msg shape: [batch_size, num_timesteps, num_edges, 2*num_dims]
        ...
        for i in range(start_idx, len(self.msg_fc2)):
            #iterate for all edge types
            msg = F.relu(self.msg_fc1[i](pre_msg))
            msg = F.dropout(msg, p=self.dropout_prob)
            msg = F.relu(self.msg_fc2[i](msg))
            msg = msg * single_timestep_rel_type[:, :, :, i:i + 1] #element-wise multiplication
            all_msgs += msg #aggreate messages from different edge types

        # Aggregate all msgs to receiver
        agg_msgs = all_msgs.transpose(-2, -1).matmul(rel_rec).transpose(-2, -1)
        # shape: [batch_size, num_timesteps, num_atoms, msg_out_shape]
        agg_msgs = agg_msgs.contiguous()

        # Skip connection
        aug_inputs = torch.cat([single_timestep_inputs, agg_msgs], dim=-1)
        #shape: [batch_size, num_timesteps, num_atoms, num_dims+msg_out_shape]

        # Output MLP
        pred = F.dropout(F.relu(self.out_fc1(aug_inputs)), p=self.dropout_prob)
        pred = F.dropout(F.relu(self.out_fc2(pred)), p=self.dropout_prob)
        pred = self.out_fc3(pred)
        #pred shape: [batch_size, num_timesteps, num_atoms, num_dims]
    ...
    def forward(self, inputs, rel_type, rel_rec, rel_send, pred_steps=1):
        # NOTE: Assumes that we have the same graph across all samples.
        """
        inputs: including current and future timesteps
        shapes of args:
          inputs: [num_sims, num_atoms, num_timesteps, num_dims]
          rel_type: [num_sims, num_edges, num_edgetypes]
          rel_rec & rel_send: [num_edges, num_atoms]

        e.g. inputs timesteps: t0,t1,t2,t3,t4
             predicted timesteps: t1',t2',t3',t4',
        """
        inputs = inputs.transpose(1, 2).contiguous()
        #shape: [num_sims, num_timesteps, num_atoms, num_dims]

        sizes = [rel_type.size(0), inputs.size(1), rel_type.size(1),
                 rel_type.size(2)]
        rel_type = rel_type.unsqueeze(1).expand(sizes)
        #expand rel_type in timesteps, the edge types keep constant for
        # all timesteps of one simulation
        #rel_type shape: [num_sims, timesteps, num_edges, num_edgetypes]
        ...
        for step in range(0, pred_steps):
            last_pred = self.single_step_forward(last_pred, rel_rec, rel_send,
                                                 curr_rel_type)
            #last_pred shape: [num_sims, num_timesteps, num_atoms, num_dims]
...
class RNNDecoder(nn.Module):
    ...
    def __init__(self, n_in_node, edge_types, n_hid,
        ...
        """
         args:
           n_in_node: number of features: location & velocity
           edge types: number of infered edge types
           n_hid: number of hidden units
        """
    ...
    def single_step_forward(self, inputs, rel_rec, rel_send,
        ...
        """
        args:
          inputs: shape [num_sims,num_node, n_in_node]
          rel_rec: receiver matrix, shape |E|*|V|
          rel_send: sender matrix, shape |E|*|V|
          rel_type: [num_sims ,num_edges, num_edgetypes]
          hidden: [num_sims,  num_atoms, n_hid]
        """
        receivers = torch.matmul(rel_rec, hidden)
        # shape: [num_sims, num_edges, n_hid]
        senders = torch.matmul(rel_send, hidden)
        # shape: [num_sims, num_edges, n_hid]
        pre_msg = torch.cat([senders, receivers], dim=-1)
        # shape: [num_sims, num_edges, 2*n_hid]
        all_msgs = Variable(torch.zeros(pre_msg.size(0), pre_msg.size(1),
                                        self.msg_out_shape))
        if inputs.is_cuda:
            all_msgs = all_msgs.cuda()
