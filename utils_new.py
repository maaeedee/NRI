...
def my_softmax(x, axis=-1):
    ...
    # return a tensor rather than a Variable
    #trans_input = x.transpose(axis, 0).contiguous()
    soft_max_1d = F.softmax(x, dim=-1) #?
...
def gumbel_softmax(logits, tau=1, hard=False, eps=1e-10):
    ...
    if hard:
        ...
        y = y_hard.zero_().scatter_(-1, k.view(shape[:-1] + (1,)), 1.0)
...
def load_data(batch_size=1, suffix=''):
    ...
    edges_train = torch.from_numpy(edges_train)
    #feat_valid = torch.FloatTensor(feat_valid)
    feat_valid = torch.from_numpy(feat_valid)
    #edges_valid = torch.LongTensor(edges_valid)
    edges_valid = torch.from_numpy(edges_valid)
    #feat_test = torch.FloatTensor(feat_test)
    feat_test = torch.from_numpy(feat_test)
    #edges_test = torch.LongTensor(edges_test)
    edges_test = torch.from_numpy(edges_test)
...
def load_motion_data(batch_size=1, suffix=''):
    ...
    edges_train = torch.from_numpy(np.array(edges_train, dtype=np.int64))
    #feat_valid = torch.FloatTensor(feat_valid)
    feat_valid = torch.from_numpy(feat_valid)
    #edges_valid = torch.LongTensor(np.array(edges_valid, dtype=np.int64))
    edges_valid = torch.from_numpy(np.array(edges_valid, dtype=np.int64))
    #feat_test = torch.FloatTensor(feat_test)
    feat_test = torch.from_numpy(feat_test)
    #edges_test = torch.LongTensor(np.array(edges_test, dtype=np.int64))
    edges_test = torch.from_numpy(np.array(edges_test, dtype=np.int64))
